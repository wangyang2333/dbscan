...
void cudaDbscanDriver::clusterVector(vector<vector<double>> currentVector, vector<int> currentIndex,
...
void cudaDbscanDriver::dbscanClustering(sensor_msgs::PointCloud &PCL) {
    /*Open PCL channels*/
    PCLforOutput = PCL;
...
void cudaDbscanDriver::ellipseSearch(vector<vector<double>>& currentResult, vector<int>& currentIndex,
